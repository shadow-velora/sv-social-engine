...
{lecons}
L'image jointe = la grille actuelle. Décide le PROGRAMME DE LA SEMAINE (RÈGLE FONDATRICE 03/08, non négociable : exactement 3 contenus — 1 POST SIMPLE + 2 CARROUSELS — publiés lundi, mercredi, vendredi, jamais le week-end) : quelles robes mettre en avant (varie par rapport à la grille), quel format pour le post simple (mannequin_pipeline, buste_produit ou robe_posee = respiration sans mannequin) et 2 briefs carrousel selon les RECETTES DE LA FONDATRICE (3 slides minimum, 5 maximum ; jamais d'images disparates — un carrousel = UN shooting cohérent) : carrousel_lineup = LA recette de ses exemples 'Carousel 1-5' (PRIORITAIRE, au moins 1 des 2 carrousels chaque semaine) : un panorama studio, la même muse répétée 3 fois, une robe par silhouette, numéros manuscrits, découpé en slides continues ; carrousel_porte_pose = UNE robe portée puis posée sur le fauteuil puis à plat, même lumière ; carrousel_lookbook = même muse en studio, une robe par slide alignée. Choisis une recette différente pour chacun des 2 carrousels et varie robes et couleurs.
Réponds UNIQUEMENT en JSON: {{"strategie": "2 phrases max", "plan_prochain_lot": [{{"robe": "nom exact du catalogue", "format": "mannequin_pipeline, buste_produit, robe_posee, carrousel_lineup, carrousel_porte_pose ou carrousel_lookbook", "concept": "nom du concept créatif choisi", "ambiance": "brief shooting en 1-2 phrases (lieu, lumière, idée forte)"}}], "ton_legendes": "1 phrase"}}""",
              montage, listing, key)
