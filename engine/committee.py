...
{lecons}
L'image jointe = la grille actuelle. Décide le PROGRAMME DE LA SEMAINE (RÈGLE FONDATRICE 03/08, non négociable : exactement 3 contenus — 1 POST SIMPLE + 2 CARROUSELS — publiés lundi, mercredi, vendredi, jamais le week-end) : quelles robes mettre en avant (varie par rapport à la grille), quel format pour le post simple (mannequin_pipeline, buste_produit ou robe_posee = respiration sans mannequin) et 2 briefs carrousel selon les RECETTES DE LA FONDATRICE (jamais d'images disparates — un carrousel = UN shooting cohérent) : carrousel_lookbook = la même muse en studio, une robe par slide, pose et cadrage identiques « Look 1, 2 or 3 ? » ; carrousel_porte_pose = UNE robe portée puis posée sur le fauteuil puis à plat, même lumière. Choisis idéalement une recette différente pour chacun des 2 carrousels et varie robes et couleurs.
Réponds UNIQUEMENT en JSON: {{"strategie": "2 phrases max", "plan_prochain_lot": [{{"robe": "nom exact du catalogue", "format": "mannequin_pipeline, buste_produit, robe_posee, carrousel_lookbook ou carrousel_porte_pose", "concept": "nom du concept créatif choisi", "ambiance": "brief shooting en 1-2 phrases (lieu, lumière, idée forte)"}}], "ton_legendes": "1 phrase"}}""",
              montage, listing, key)
